...
class ConcurrentRotatingFileHandler(BaseRotatingHandler):
    ...
    def __init__(self, filename, mode='a', maxBytes=0, backupCount=0,
        ...
        try:
            ...
            # Due to a different logging release without encoding support
            # (Python 2.4.1 and earlier?)
        ...
        # Prevent multiple extensions on the lock file (Only handles the normal
        # "*.log" case.)
        ...
        # For debug mode, swap out the "_degrade()" method with a more a verbose
        # one.
        if debug:
    ...
    def doRollover(self):
        ...
        self.stream.close()
        try:
            # Attempt to rename logfile to tempname:  There is a slight race-
            # condition here, but it seems unavoidable
            tmpname = None
